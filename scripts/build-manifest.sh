...
if [[ "$NETWORK" == "remote" ]]; then
    REMOTE_BLOCK=$(curl -X POST -H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","method":"eth_blockNumber","id":1}' http://18.223.196.75 | node_modules/node-jq/bin/jq ".result" | tr -d '"')
